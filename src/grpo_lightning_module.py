        ...
        if self.grpo_trainer:
            ...
            if self.grpo_trainer.original_model_state is None:
                ...
                if not trainable_param_names:
                    print("!!! 警告: 模型中未找到任何可训练参数 !!!")
                else:
                    print(f"✅ 在模型中找到 {len(trainable_param_names)} 个可训练参数。请从下面的列表中选择一个用于追踪：")
                    # 只打印前30个，避免刷屏
                    for i, name in enumerate(trainable_param_names[:30]):
                        print(f"  {i+1}: {name}")
                
                print("="*50 + "\n")
                # 诊断日志
                print(f"✅ [on_train_start] 原始模型状态已成功保存。")
                print(f"   -> 找到 {len(params_found)} 个需要梯度的参数。")
                if len(params_found) == 0:
                    print("   ⚠️ 警告: 未找到需要梯度的参数！")
                else:
                    # 打印一些示例键来验证
                    example_keys = list(params_found.keys())[:5]
                    print(f"   -> 示例参数名: {example_keys}")
 
    # 在 GRPOLightningModule 类中...

# 💡【终极调试代码】请添加这两个回调函数来检查优化器的核心功能

    def on_before_optimizer_step(self, optimizer):
        """在优化器更新权重之前被调用，用于检查梯度并记录更新前的值"""
        # 初始化检查变量
        self._param_before_update = None
        self._grad_norm = None
        
        # 找一个模型中确定存在的、可训练的参数
        # 这个参数名来自您之前的日志，应该是存在的
        param_to_track_name = "model.model.mlp_in_X.0.weight" 
        
        try:
            # 获取我们要追踪的参数
            target_param = dict(self.named_parameters())[param_to_track_name]

            # 1. 检查梯度
            if target_param.grad is not None:
                # 计算梯度的模长 (L2 norm)，这是一个衡量梯度大小的标量
                self._grad_norm = torch.linalg.norm(target_param.grad).item()
            # else: self._grad_norm 保持为 None
                
            # 2. 记录更新前的参数值
            self._param_before_update = target_param.data.clone()

        except KeyError:
            print(f"⚠️ [DEBUG] 警告: 找不到要追踪的参数 '{param_to_track_name}'")


    def on_after_optimizer_step(self, optimizer):
        """在优化器更新权重之后被调用，用于比较参数值的变化"""
        # 如果更新前的值没有被记录，则跳过
        if self._param_before_update is None:
            return

        param_to_track_name = "model.model.mlp_in_X.0.weight" # 确保和上面一致

        try:
            # 再次获取同一个参数
            target_param = dict(self.named_parameters())[param_to_track_name]
            value_after = target_param.data.clone()
            ...
            if self._grad_norm is not None:
                print(f"✅ 梯度存在，模长为: {self._grad_norm}")
                if self._grad_norm == 0.0:
                    print("   -> 警告: 梯度为零！这意味着损失函数在该点的导数为0。")
            else:
                print("   -> ❌ 错误: 梯度为 None! 这意味着反向传播失败或被跳过。")

            # 报告权重更新检查结果
            if torch.equal(self._param_before_update, value_after):
                print("❌ 核心错误: 优化器步骤【没有】改变权重!")
            else:
                print("✅ 成功: 优化器步骤【已经】改变了权重。")
                diff = torch.linalg.norm(self._param_before_update - value_after).item()
                print(f"   -> 权重变化量 (L2 Norm): {diff}")
            
            print("="*30 + "\n")

        except KeyError:
            # 这个错误不应该发生，因为 on_before_optimizer_step 已经检查过了
            pass
# “黄金标准”版的 configure_optimizers
