...
def main(cfg: DictConfig):
    ...
    if not cfg.general.test_only:
        ...
        print(f"使用随机种子进行测试: {random_seed}")
    ...
    if dataset_config["name"] in [
    ...
    elif dataset_config["name"] == "my_tree":  # 新的数据集 my_tree
        ...
        from analysis.visualization import NonMolecularVisualization
        from analysis.spectre_utils import (
            TreeSamplingMetrics,  # 可以使用现有的 Tree 评估指标
        )

        datamodule = MyTreeGraphDataModule(cfg)
        sampling_metrics = TreeSamplingMetrics(datamodule)  # 重用 Tree 的评估指标
    ...
    elif dataset_config["name"] == "my_planar":  # 新的数据集 my_planar
        ...
        from analysis.visualization import NonMolecularVisualization
        from analysis.spectre_utils import (
            PlanarSamplingMetrics,  # 可以使用现有的 Planar 评估指标
        )

        datamodule = MyPlanarGraphDataModule(cfg)
        sampling_metrics = PlanarSamplingMetrics(datamodule)  # 重用 Planar 的评估指标
