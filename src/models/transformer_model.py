...
class GraphTransformer(nn.Module):
    ...
    def forward(self, X, E, y, node_mask):
        ...
        # 🎯 第一阶段：治本 - 使用激活检查点降低激活值累积
        # 这是解决显存瓶颈的核心：每个transformer层都使用检查点
        def checkpoint_layer(layer_fn, x_input, e_input, y_input, mask_input):
            """激活检查点包装函数，用于单个transformer层"""
            return layer_fn(x_input, e_input, y_input, mask_input)
        
        for layer in self.tf_layers:
            # 应用激活检查点：牺牲计算时间换取显存空间
            X, E, y = checkpoint(
                checkpoint_layer,
                layer, X, E, y, node_mask,
                use_reentrant=True  # 使用可重入版本以确保兼容性
            )
